    ...
    RARE_ITEMS.forEach((item, index) => {
        if (shuffled[index]) {
            if (item.name === 'totem_dore') {
                // Totem doré : juste un compteur, pas de position
                positions[item.name] = {
...
async function startGame() {
    ...
    // Tirer la première mission temporaire
...
function changeWeather() {
    ...
    // Vérifier si la mission temporaire a forcé le beau temps
...
// ========================================
// PARTIE 8 : MISSIONS TEMPORAIRES
// ========================================

/**
 * Tire une nouvelle mission temporaire
 */
function drawTemporaryMission() {
    console.log('\n--- DÉBUT drawTemporaryMission() ---');
    console.log('🎯 Tirage d\'une nouvelle mission temporaire...');
    ...
    console.log(`🎯 Mission temporaire #${mission.id} : ${mission.libelle}`);
...
/**
 * Met à jour l'affichage de la mission temporaire
 */
...
function checkTemporaryMission(player, missionId) {
    ...
    // Attribuer les votes
    const votes = 25;
    player.addVotes(votes, `Mission temporaire #${missionId}`);
    ...
    showToast(`${player.picto} 🎯 Mission temporaire accomplie ! +${votes} votes`, 'success', 5000);
