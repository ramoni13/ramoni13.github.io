...
function confirmFouille() {
    ...
    if (globalData.itemsRares) {
        for (let itemName in globalData.itemsRares) {
            ...
            if (itemData.position && itemData.position.x === x && itemData.position.y === y && !itemData.found) {
...
function pickupRareItem(itemName, x, y) {
    ...
    // Marquer comme trouvé dans Firebase
    if (globalData.itemsRares && globalData.itemsRares[itemName]) {
        globalData.itemsRares[itemName].found = true;
        globalData.itemsRares[itemName].foundBy = player.name;
    ...
    if (itemName === 'totem_dore') {
        const votes = rareItem.baseVotes || 250;
        player.addVotes(votes, 'Totem Doré');
        showToast(`${player.picto} 🏆 TOTEM DORÉ ! +${votes} votes !`, 'success', 5000);
