...
function confirmFouille() {
    ...
    // Chercher les items rares (sauf totem doré)
    ...
    if (globalData.itemsRares) {
        for (let itemName in globalData.itemsRares) {
            ...
            // Vérifier que l'item a une position et n'est pas possédé
            if (itemName !== 'totem_dore' && 
                itemData.position && 
                itemData.position.x === x && 
                itemData.position.y === y && 
                !itemData.possessedBy) {
...
async function releaseRareItem(itemName, reason) {
    ...
    players.forEach(player => {
        if (player.rareItemPossessed === itemName) {
...
/**
 * Libère tous les items rares possédés (fin de partie)
 */
async function releaseAllRareItems() {
    console.log('🔓 Libération de tous les items rares...');
    
    if (!globalData.itemsRares) return;
    
    for (let itemName in globalData.itemsRares) {
        const itemData = globalData.itemsRares[itemName];
        if (itemData.possessedBy && itemName !== 'totem_dore') {
            await releaseRareItem(itemName, 'Fin de partie');
        }
    }
}
...
function showMissionFlashStep1(player) {
    ...
    html += '<p style="font-size: 1.3rem; font-weight: bold; margin: 20px 0;">Piochez 1 carte Action</p>';
    html += '<p style="font-size: 0.9rem; color: #666; margin: 15px 0;">Vous allez devoir réaliser une mission flash devant la caméra</p>';
    ...
    speak('Caméra trouvée ! Piochez une carte Action.');
}
...
function showMissionFlashStep2() {
    ...
    let html = '<h3 style="color: #9b59b6; margin-top: 0;">🎬 MISSION FLASH</h3>';
