...
function showLieuxMystiquesInteraction() {
    ...
    for (let lieuName in globalData.lieuxMystiques) {
        ...
        if (lieu && pos) {
            ...
            html += `<button onclick="activateLieu('${lieuName}')" class="btn" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; text-align: left; padding: 15px; border: none; border-radius: 10px; cursor: pointer; transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.02)'" onmouseout="this.style.transform='scale(1)'">`;
            html += `<div style="display: flex; align-items: center; gap: 10px;">`;
            html += `<div style="font-size: 2rem;">${lieu.icon}</div>`;
            html += `<div style="flex: 1;">`;
            html += `<div style="font-weight: bold; font-size: 1rem; margin-bottom: 3px;">${lieu.name.replace('_', ' ').toUpperCase()}</div>`;
...
function activateAutelOffrandes(player, lieu) {
    ...
    if (hasMangue) {
        ...
        showToast(`${player.picto} ${lieu.icon} Autel : +${votes} votes + 1 carte Action !`, 'success', 4000);
        speak(`Autel des Offrandes ! Plus ${votes} votes et piochez 1 carte Action !`);
...
function activateStationMeteo(player, lieu) {
    ...
    showToast(`${player.picto} ${lieu.icon} Station Météo : 3 tours de Beau Temps !`, 'info', 4000);
    speak('Station Météo activée ! Les 3 prochains tours seront ensoleillés !');
